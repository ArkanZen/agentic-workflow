...
## 第 0 步：定位工作流仓库（全局命令必读）

本命令可能被装在宿主全局（`~/.claude/commands/wf-install.md`），此时当前项目可能**还没有** `.agentic-workflow/manifest.json`。执行任何安装/切换/升级前，必须先确定 `<agentic-workflow-path>`（含 `install.sh` 的工作流仓库目录），按以下顺序解析，**找到即停，不要先去问用户**：
