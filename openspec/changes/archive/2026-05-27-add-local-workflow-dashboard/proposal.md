...
- 本轮复核摘要：工具能力改为只读版本检测，移除 OpenSpec/GStack/Superpowers 更新入口；新增本地项目安装工作流属于受控写操作，必须复用 `install.sh --target --type --no-interactive`，并限制目标目录在用户配置扫描根内。
- 本轮布局复核摘要：左侧栏从项目目录升级为全局主导航，安装工作流、工具能力、扫描设置和工作流说明成为全局页面；项目详情只保留项目作用域功能，避免把全局安装动作误绑定到已安装项目。
