...
- **下沉参考内容**：GStack 安装说明、OpenSpec 集成细节（opsx 别名、config 私有键、多方共写）从常驻块移到 README「集成说明」（按需查，不必每轮加载）。
- **压缩常驻段**：GStack 命令名全表 + 审查降级链压成指向 manifest 的精简版（权威数据本就在 `gstackCommandMap`/`reviewFallback`）。
- **预算护栏**：`validate-workflow.sh` 新增「常驻块预算」检查（默认 ≤70 行），超了告警，逼非普适内容下沉到 wf-* 命令或 README，防止常驻面再悄悄膨胀。

## 1.7.1 - 2026-06-29

把 v1.7.0 的 Agent 角色编排"做轻"：评估后认为常驻的角色表与工作流"留痕可追踪/一致性"初心有张力，且轻通道用不到却要常驻加载。

- 从 CLAUDE.md/AGENTS.md 常驻块移除「Agent 角色编排」表，改为 `/wf-complex` 内一句**临场优化提示（非正式契约）**——省常驻 token，明确它不是工作流契约。
- 提示中新增一条原则：子 agent 内部过程不落盘，**关键产出须写回 proposal/design/tasks 等 OpenSpec 工件以保持可追踪**（守住"留痕"初心）。
- 审查降级链（v1.7.0）保留不变，仍是降耦合的核心。
