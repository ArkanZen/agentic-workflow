...
| 双宿主适配 | 同时安装 Claude Code 命令和 Codex App skills，GStack 命令按宿主自动映射 |
...
| 本地 Dashboard | `dashboard/` 提供 Node Web UI，集中查看本机项目安装状态、OpenSpec 统计和 doctor 结果 |
| 中文工件规范 | 独立设计文档文件名使用中文描述，OpenSpec change 目录继续使用 kebab-case，兼顾可读性和 CLI 兼容 |

## 近期版本重点

这些能力来自本地开发日志和近期提交，属于当前 README 需要优先传达的行为：

| 版本 | 用户可感知变化 |
|------|----------------|
| `1.1.10` | 各档位模板补齐中文文件命名规范和绝对路径展示规则，doctor 发布校验同步覆盖 |
| `1.1.9` | 命名规范从 `naming:` 移入 `context:`，确保 OpenSpec CLI 生成工件时能真正读取中文命名规则 |
| `1.1.8` | Codex `/wf-*` 增加启动自检、强制依赖加载和收尾审计，不再只按方法论摘要推进 |
| `1.1.7` | 工作流生成工件后必须展示完整绝对路径，便于 Codex App 直接打开本地文件 |
| `1.1.6` | 升级检测迁移到 GitHub Releases，`/wf-install` 优先通过 manifest 中的 `sourceRepo` 查询最新 release tag |
| `1.1.5` | Codex 和 Claude 的关键确认节点对齐，proposal / design / tasks 生成后先展示摘要再进入实现 |
...
如果你维护多个本地项目，可以启动 Node Web UI 查看每个项目的工作流安装状态、workflow tier（工作流档位）、版本、宿主、OpenSpec active change（活跃变更）统计、GStack/Superpowers 工具能力和 doctor（健康检查）结果。
...
默认打开 `http://127.0.0.1:5173`。本地 API 只监听 `127.0.0.1`，写操作仅允许预定义 workflow actions（工作流维护动作）：运行 doctor（健康检查）、upgrade（升级）当前档位模板、switch tier（切换工作流档位）。MVP 不提供远程访问、账号体系、插件市场或第三方插件自动安装。
...
| 你要做什么 | 推荐方式 |
|------------|----------|
| 首次给一个项目接入工作流 | 让 AI 读取 README 后运行 `install.sh`，或手动交互安装 |
| 在脚本或 AI 自动化中安装 | `install.sh --type <档位> --target <目录> --no-interactive` |
| 已安装项目升级模板 | 在目标项目中运行 `/wf-install`，优先使用 manifest 的远程版本检测 |
| 项目类型变化，需要换档位 | 在目标项目中运行 `/wf-install` 进入 SWITCH 模式，或手动 `install.sh --switch` |
