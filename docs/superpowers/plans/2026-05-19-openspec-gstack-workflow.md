...
**Goal:** 在 `arkan-workflow` 中创建全套工作流模板（OpenSpec 配置、Claude 命令、Codex skills、GStack 移植 skill），并完成 `plutus-daliy-report` 项目的初始化接入。

**Architecture:** 所有模板集中维护于 `arkan-workflow/templates/`，各项目通过拷贝安装。`openspec/config.yaml` 的 `rules:` 字段注入 GStack gate 逻辑（AI 自我执行），两个 CLI 通过同名 skill 实现审查对等，共享同一份 `openspec/` 工件目录。
...
arkan-workflow/README.md             [CREATE] Task 11
...
**Files:**
- Create: `arkan-workflow/templates/openspec/config-backend.yaml`

- [ ] **Step 1: 创建目录结构**

```bash
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/openspec
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/claude/commands
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/wf
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/openspec-quick
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/gstack-plan-eng-review
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/gstack-cso
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/gstack-review
mkdir -p /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/gstack-plan-design-review
```
...
写入文件 `arkan-workflow/templates/openspec/config-backend.yaml`，内容如下：
...
```bash
cat /Users/ryan/aiworkspace/arkan-workflow/templates/openspec/config-backend.yaml | head -5
```
...
```bash
cd /Users/ryan/aiworkspace/arkan-workflow
...
**Files:**
- Create: `arkan-workflow/templates/openspec/config-frontend.yaml`

- [ ] **Step 1: 写入 config-frontend.yaml**

写入文件 `arkan-workflow/templates/openspec/config-frontend.yaml`，内容如下（在后端版基础上，design rules 增加 /plan-design-review 门控）：
...
**Files:**
- Create: `arkan-workflow/templates/claude/commands/wf.md`

- [ ] **Step 1: 写入 wf.md**

写入文件 `arkan-workflow/templates/claude/commands/wf.md`：
...
**Files:**
- Create: `arkan-workflow/templates/claude/commands/openspec-quick.md`

- [ ] **Step 1: 写入 openspec-quick.md**

写入文件 `arkan-workflow/templates/claude/commands/openspec-quick.md`：
...
**Files:**
- Create: `arkan-workflow/templates/codex/skills/wf/SKILL.md`

- [ ] **Step 1: 写入 wf SKILL.md**

写入文件 `arkan-workflow/templates/codex/skills/wf/SKILL.md`：
...
**Files:**
- Create: `arkan-workflow/templates/codex/skills/openspec-quick/SKILL.md`

- [ ] **Step 1: 写入 openspec-quick SKILL.md**

写入文件 `arkan-workflow/templates/codex/skills/openspec-quick/SKILL.md`：
...
- Create: `arkan-workflow/templates/codex/skills/gstack-plan-eng-review/SKILL.md`
...
写入文件 `arkan-workflow/templates/codex/skills/gstack-plan-eng-review/SKILL.md`。
...
```bash
wc -l /Users/ryan/aiworkspace/arkan-workflow/templates/codex/skills/gstack-plan-eng-review/SKILL.md
```
...
- Create: `arkan-workflow/templates/codex/skills/gstack-cso/SKILL.md`
...
写入文件 `arkan-workflow/templates/codex/skills/gstack-cso/SKILL.md`。
...
- Create: `arkan-workflow/templates/codex/skills/gstack-review/SKILL.md`
...
写入文件 `arkan-workflow/templates/codex/skills/gstack-review/SKILL.md`。
...
- Create: `arkan-workflow/templates/codex/skills/gstack-plan-design-review/SKILL.md`
...
写入文件 `arkan-workflow/templates/codex/skills/gstack-plan-design-review/SKILL.md`。
...
### Task 11: 创建 arkan-workflow README.md

**Files:**
- Create: `arkan-workflow/README.md`

- [ ] **Step 1: 写入 README.md**

写入文件 `arkan-workflow/README.md`：
...
cd /Users/ryan/aiworkspace/arkan-workflow
git add README.md
git commit -m "docs: add arkan-workflow README with installation guide"
```
...
将 `plutus-daliy-report/openspec/config.yaml` 内容替换为 `arkan-workflow/templates/openspec/config-backend.yaml` 的内容。
...
git commit -m "feat: install arkan-workflow templates (GStack skills + /wf + /openspec-quick)"
```
...
- [ ] **Step 1: 验证 arkan-workflow 模板完整性**

```bash
find /Users/ryan/aiworkspace/arkan-workflow/templates -type f | sort
```
...
Tasks 1-6 可并行（均为 arkan-workflow 模板创建，互不依赖）。
