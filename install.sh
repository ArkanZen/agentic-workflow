...
# ══════════════════════════════════════════════════════════════════════════════
# --global 模式：把 /wf-install 引导命令装到宿主全局，不针对任何项目
# ══════════════════════════════════════════════════════════════════════════════
if [[ "$GLOBAL_MODE" == "true" ]]; then
  step "全局安装 /wf-install 引导命令"

  SOURCE_REPO="$(detect_source_repo)"
  [[ -z "$SOURCE_REPO" ]] && SOURCE_REPO="${AGENTIC_WORKFLOW_SOURCE_REPO:-}"

  # 写全局配置 ~/.agentic-workflow/config，供全局命令按需 clone / 复用本地仓库
  GLOBAL_CFG_DIR="$HOME/.agentic-workflow"
  mkdir -p "$GLOBAL_CFG_DIR"
  {
    echo "# agentic-workflow 全局配置（由 install.sh --global 写入）"
    echo "# 供全局 /wf-install 定位工作流仓库；可手动编辑"
    echo "sourceRepo=$SOURCE_REPO"
    echo "workflowPath=$WORKFLOW_DIR"
  } > "$GLOBAL_CFG_DIR/config"
  ok "已写入全局配置: $GLOBAL_CFG_DIR/config"
  info "sourceRepo: ${SOURCE_REPO:-（未检测到，将依赖缓存或手动指定）}"
  info "workflowPath: $WORKFLOW_DIR"

  # 决定装到哪些宿主：已存在目录或可执行命令则装；都没有则两者都装（尽力而为）
  G_CLAUDE=false; G_CODEX=false
  if [[ -d "$HOME/.claude" ]] || command -v claude &>/dev/null; then G_CLAUDE=true; fi
  if [[ -d "$HOME/.codex" ]] || command -v codex &>/dev/null; then G_CODEX=true; fi
  if [[ "$G_CLAUDE" == "false" && "$G_CODEX" == "false" ]]; then
    G_CLAUDE=true; G_CODEX=true
  fi

  if [[ "$G_CLAUDE" == "true" ]]; then
    mkdir -p "$HOME/.claude/commands"
    render_template_file "$TEMPLATES/claude/commands/wf-install.md" "$HOME/.claude/commands/wf-install.md"
    ok "Claude Code: ~/.claude/commands/wf-install.md"
  fi
  if [[ "$G_CODEX" == "true" ]]; then
    mkdir -p "$HOME/.codex/skills/wf-install"
    cp -R "$TEMPLATES/codex/skills/wf-install/." "$HOME/.codex/skills/wf-install/"
    if [[ -f "$HOME/.codex/skills/wf-install/SKILL.md" ]]; then
      _tmp_skill="$(mktemp)"
      render_template_file "$HOME/.codex/skills/wf-install/SKILL.md" "$_tmp_skill"
      mv "$_tmp_skill" "$HOME/.codex/skills/wf-install/SKILL.md"
    fi
    ok "Codex App: ~/.codex/skills/wf-install/"
  fi

  echo ""
  echo -e "  ${GREEN}${BOLD}全局安装完成。${RESET}"
  echo -e "  在任意项目里运行 ${BOLD}/wf-install${RESET} 即可安装工作流；不运行则不改动该项目。"
  echo -e "  卸载：删除 ~/.claude/commands/wf-install.md、~/.codex/skills/wf-install/ 和 ~/.agentic-workflow/"
  echo ""
  exit 0
fi
