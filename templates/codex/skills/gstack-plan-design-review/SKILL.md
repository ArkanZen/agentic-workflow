...
（Codex 替代：输出 ASCII art 线框图，说明布局结构和组件层次）
...
（跳过：gstack designer 在 Codex 中不可用。改用 ASCII art 线框图替代所有 mockup。）
...
（Codex 替代：输出 ASCII art 线框图，说明布局结构和组件层次）
Save to the same `$_DESIGN_DIR` directory.
...
（跳过：gstack-learnings-log 在 Codex 中不可用。将设计审查学习要点写入 AGENTS.md。）
