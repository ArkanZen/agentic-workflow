...
> **可选编排（临场优化，非正式契约）**：阶段多、风险高时，可酌情把各阶段交给专职子 agent——探索→Explore/Plan，设计→tech-design，实现→`superpowers:subagent-driven-development`，审查→code-reviewer，无依赖任务→`superpowers:dispatching-parallel-agents`；不必要或子 agent 不可用则主线程顺序执行（无需当作降级说明）。注意：子 agent 内部过程不落盘，关键产出仍须写回 proposal/design/tasks 等 OpenSpec 工件以保持可追踪。
>
> 所有审查统一遵循工作流块「审查降级链」：GStack 不可用时代码审查降级 /code-review、安全审查降级 /security-review（Codex 无原生则用结构化自检清单），降级必须说明。
