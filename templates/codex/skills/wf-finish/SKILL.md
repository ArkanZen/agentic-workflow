...
- 有选项的地方，优先使用 Codex App 提供的 UI 交互工具。
- 只有 UI 交互工具不可用时，才退化为文本选项。
...
3. **若存在**，读取工作流类型并处理：
   ...
   **wf-quick / wf-small / wf-complex**（有 OpenSpec change）：
   ...
   - 若 change 仍 in-progress：使用 UI 交互询问：
     - 选项 A：保留 in-progress（下次继续，.wf-active 保留）
     - 选项 B：放弃此 change（运行 `openspec abandon <change-name>`，再删除 `.wf-active`）
   - 按用户选择执行，宣告结果

4. **代码集成（有代码改动且用户确认完成时）**：加载 `superpowers:finishing-a-development-branch`，按其结构化选项决定 merge / 开 PR / cleanup。该 skill 负责 git 分支收尾，与本命令的 `.wf-active` 状态管理互补——先处理 change/状态，再用它处理代码集成。Superpowers 不可用时说明缺失并降级为手动提示。
